...
module gregs_x_carriage(with_fanmount=true) 
{
	difference()
	{
		union ()
		{
			...
			{
				rotate(45)
				assign(ehole=cos(45)*(50+lm8uu_holder_width+cos(45)*10),
					ehole_r=10,ehole_t=lm8uu_support_thickness*1.7)
				translate([0,0,ehole_t/2])
				{
					cube([ehole-2*ehole_r,ehole,ehole_t],center=true);
					cube([ehole,ehole-2*ehole_r,ehole_t],center=true);
					for(i=[-1,1])
					for(j=[-1,1])
					translate([i*(ehole/2-ehole_r),j*(ehole/2-ehole_r),0])
					cylinder(r=ehole_r,h=ehole_t,center=true);
				}

				for(i=[-1,1])
				for(j=[-1,1])
				if (!((i==-1) &&(j==1)))
				translate([23*i,18*j,(lm8uu_support_thickness*1.7+2)/2])
				cube([15,15,lm8uu_support_thickness*1.7+2],center=true);
			}
		}
